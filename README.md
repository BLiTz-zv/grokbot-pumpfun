...
[![CI](https://github.com/BLiTz-zv/grokbot-pumpfun/actions/workflows/ci.yml/badge.svg)](https://github.com/BLiTz-zv/grokbot-pumpfun/actions/workflows/ci.yml)
...
**Это бумажный стол.** По умолчанию `mode: dry-run`. `LiveExecutor` намеренно
оставлен заглушкой: код не подписывает транзакции, не ходит в Phantom и не
отправляет бандлы в Jito. Запуск в `live` этот репозиторий отклоняет, пока
заглушка на месте. Форк [BLiTz-zv/grokbot-pumpfun](https://github.com/BLiTz-zv/grokbot-pumpfun)
(upstream — zostaff/grokbot-pumpfun).
...
```bash
git clone https://github.com/BLiTz-zv/grokbot-pumpfun.git
cd grokbot-pumpfun
...
grokbot run                  # dry-run, бумажный стол
```
...
| `solana` | RPC и поля кошелька/Jito. В этом репозитории не используются: live — заглушка |
| `market` | комиссия кривой, потолок влияния заявки, тонкая кривая, стоимость круга |
...
`tx_hash: "dry_run"`. Цена входа — средняя цена исполнения по кривой
(комиссия и влияние своей заявки), а не спот. Стоп-лосс смотрит на спот
как на движение рынка; PnL закрытия — снова по кривой, не по котировке.

`mode: live` этот репозиторий не запускает: `LiveExecutor` поднимает
`NotImplementedError`. Флаг `--i-understand-the-risk` понадобится только
если исполнение когда-нибудь допишут отдельно — не здесь.
...
Практический смысл в одной строке: dry-run — это и есть стол. Если он
покупает по котировке, отчёт показывает прибыль, которой на кривой нет.
...
**След намерения купить.** Запись `intent` пишется до исполнения. Если
процесс умрёт между расчётом заявки и учётом, на диске останется намерение
без покупки — на следующем старте это выводится. В dry-run кошелька нет:
это след сбоя бумажного стола, а не ончейн-позиция.
...
`DryRunExecutor` работает полностью по математике кривой. `LiveExecutor.buy`
и `.sell` поднимают `NotImplementedError`. Код, подписывающий транзакции
приватным ключом, сюда не принимается: этот репозиторий — бумажный стол.
...
Работайте в `dry-run`. Live-часть не дописана намеренно и в этом форке
не будет: дописывая её у себя, вы принимаете ответственность за то, что
