...
grokbot run                   # бумажный стол (dry-run)
```
...
Только после этого имеет смысл смотреть, что отсекается зря. Live в этом
репозитории не включается: `LiveExecutor` — заглушка.
...
Процесс умер между расчётом заявки и учётом позиции. В `dry-run` кошелька
нет: это след сбоя бумажного стола, а не ончейн-токены. Если когда-нибудь
включите live у себя — тогда да, проверьте кошелёк руками.
...
Включён `mode: live`, но `LiveExecutor` — заглушка. Этот репозиторий
такой запуск теперь отклоняет ещё на старте. Верните `mode: dry-run`.
...
## Чек-лист бумажного стола

- [ ] `mode: dry-run`, `grokbot doctor` зелёный (или с замечаниями, без fail)
- [ ] сутки в dry-run отработаны, `replay` разобран
- [ ] PnL в логе — с комиссией и влиянием заявки, не по споту
- [ ] `LiveExecutor` по-прежнему заглушка; live этот репозиторий не запускает
...
Не включайте live в этом форке. Код, который подписывает транзакции
приватным ключом, сюда не принимается.
