...
- **Замок экземпляра** писался без `O_EXCL`: два процесса могли оба решить,
  что файл их.
- **Живость считалась только по токенам, пережившим фильтр.** 94% потока
  кончается skip на мониторе, и через десять минут `/healthz` помечал
  стол `stalled`, хотя сокет писал события. Теперь считается любое
  сообщение монитора.
